    ...
    <div className="space-y-8 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Overview of student ranks based on cumulative points and averages.
        </p>
      </div>

      {/* Filter tabs */}
      <div className="flex gap-1.5 p-1 bg-slate-205/50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl w-fit">
          ...
          <button
            ...
            className={`px-4 py-2 text-xs font-bold rounded-lg uppercase tracking-wider transition ${
              timeframe === tf
                ...
                : "text-slate-550 dark:text-slate-450 hover:bg-white/50"
            }`}
          >
            {tf}
          </button>
      ...
      {loading ? (
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
                {leaderboard.map((entry) => {
                  let rankCell = (
                    <span className="font-extrabold text-slate-550">{entry.rank}</span>
                  );
                  if (entry.rank === 1) {
                    rankCell = (
                      <span className="flex items-center justify-center w-7 h-7 bg-amber-500/10 border border-amber-500/30 text-amber-555 rounded-full font-black text-xs">
                        <Medal className="w-3.5 h-3.5 text-amber-500" />
                      </span>
                    );
                  } else if (entry.rank === 2) {
                    rankCell = (
                      <span className="flex items-center justify-center w-7 h-7 bg-slate-100 dark:bg-slate-800 border border-slate-250 dark:border-slate-700 text-slate-450 rounded-full font-black text-xs">
                        2
                      </span>
                    );
                  } else if (entry.rank === 3) {
                    rankCell = (
                      <span className="flex items-center justify-center w-7 h-7 bg-amber-800/10 border border-amber-800/20 text-amber-850 dark:text-amber-600 rounded-full font-black text-xs">
                        3
                      </span>
                    );
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-6">
                        ...
                        <span className="text-[10px] text-slate-450">ID: #{entry.user_id}</span>
                      </td>
        ...
        <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl text-center p-6 text-slate-400">
          ...
          <p className="text-sm text-slate-500 max-w-sm mt-1">
            Attempts will appear once students complete quizzes.
          </p>
        </div>
      )}
    </div>
